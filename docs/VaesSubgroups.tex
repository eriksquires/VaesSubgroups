...
one by amplified slopes (pain, neurocognitive), with high-end
convergence after accounting for a single influential cluster. This
subgroup's symptom trends could align with those of fibromyalgia.
...
grouping choices in \hyperref[sec-loo]{Leave-one-out Analysis}
...
increases, all other symptom domains rise together. Important to note
C36 as an outlier for the entire cohort. Without it the upper limits of
PEM, Neurocognitive and Pain would match around C7 and C26.
...
Based on the trends noted in Section~\ref{sec-different}, a discussion
of whether the low-intensity group could relate to fibromyalgia is
warranted. We note that Vaes\textsuperscript{1} did not model
...
Vaes\textsuperscript{1} recruited participants who self-reported ME/CFS
and later mapped them to multiple case definitions, yielding a broad
...
clusters\textsuperscript{6}. These were mostly of size 1, with 4
...
pain and neurocognitive limits as described in Section~\ref{sec-li}.
...
FM enrichment as a downstream hypothesis, but the PEM-referenced
structure stands independently of comorbidity.
