...
We use the Vaes cluster data[@vaes2023_excel] for our analysis. From this Excel file we manually:

-   Remove lines above cluster IDs
-   Remove row with number of participants
-   Remove frequency rows (- f), leaving only severity (- s)
-   Remove frequency and severity suffixes from symptom names
-   Remove standard deviation columns
-   Shorten cluster identifiers to "C" and their number
-   Replace "mean" with the new short cluster ID
-   Save the resulting file as vaes_clusters_severity.csv

From this file we used R to:
...
This last file is what we then used for our analysis, and the plots. We present the final data used below. Values were rounded and abbreviations used for formatting.
