...
An examination of the charts above show the low-intensity group occupies a very narrow range of PEM intensity and then stops abruptly when PEM reaches \~2.9. We see no reason for a PEM specific boundary but the charts in @fig-pain suggest that our group is not PEM limited but pain and neurocognitive limited. Pain and neurocognitive symptoms trend upward and then the clusters stop at C7 just before they would cross the high-intensity group trend lines. This truncation could be due to diagnostic criteria which would reclassify these individuals as fibromyalgia after the pain and neurocognitive scores cross the high-intensity lines. Doing similar visual inspections with fibromyalgia cohorts could confirm this.
...
We can also quantify the significance of the group distinction using analysis of covariance (ANCOVA) in order to identify the domains which are most discriminatory. We compare our two groups for each symptom domain. 
...
if (!file.exists(input_file)) {
 ...
 ancova_results <- read_csv(here::here("data", "ancova_results.csv")) %>%
  mutate(p_value = round(p_value, 3)) %>%
   ...
   rename(`F Value`= F_value)

  gt(ancova_results %>% select(Domain, `F Value`, `P Value`, significance))
```
:::

@tbl-t4 shows agreement with the charts that other, neuroendocrine and autonomic domains show the most significant offsets between the two groups. The high F and P values for fatigue, however, do not agree with the charts. As we note earlier in @sec-similar, the lines in @fig-fatigue are nearly identical but R² is significantly weaker in the low-intensity group. The high F and P values in the ANCOVA analysis for fatigue is most likely an artifact of the high spread in the low-intensity group. 

**Statistical Summary**
These three analyses strongly support the existence of two distinct subgroups within the Vaes clusters while the ANCOVA analysis also helps identify which domains can best identify the two subgroups.
